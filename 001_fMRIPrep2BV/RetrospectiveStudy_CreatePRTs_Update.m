function [PRT] = RetrospectiveStudy_CreatePRTs_Update


%% Gather basic information needed to begin PRT creation.
prompt = {'Did you drop volumes in your fMRI anaylsis?','Enter the subject IDs that you want to analyze.','Right Hand Movements','Right Foot Movements','Left Hand Movements', 'Left Foot Movements', 'Tongue Movements','Boston Naming','Verb Generation','VF Category','VF Letters','Definition Naming','Picture Naming'};
dlgtitle = 'User Provided PRT Input Parameters';
dims = [1 75];
definput = {'Put 0 if no; otherwise provide # of vols dropped.','E.g., 1, 3, 5, 8, or 1:15,22,45:50','Yes (1) or No (0)','Yes (1) or No (0)','Yes (1) or No (0)','Yes (1) or No (0)','Yes (1) or No (0)','Yes (1) or No (0)','Yes (1) or No (0)','Yes (1) or No (0)','Yes (1) or No (0)','Yes (1) or No (0)','Yes (1) or No (0)'};
...
for subi = 1:length(PRT.Subs2process)
    ...
    if PRT.Right.HandMove.Create == 1
        ...
        tmpprt.SaveAs(fullfile(PRT.PRTSavePath,[subID '_MotRH_Run1.prt']));
    end
    ...
    if PRT.Right.FootMove.Create == 1
        ...
        tmpprt.SaveAs(fullfile(PRT.PRTSavePath,[subID '_MotRF_Run1.prt']));
    end
    ...
    if PRT.Left.FootMove.Create == 1
        ...
        tmpprt.SaveAs(fullfile(PRT.PRTSavePath,[subID '_MotLF_Run1.prt']));
    end
    ...
    if PRT.Left.HandMove.Create == 1
        ...
        tmpprt.SaveAs(fullfile(PRT.PRTSavePath,[subID '_MotLH_Run1.prt']));
    end
    ...
    if PRT.TongueMove.Create == 1
        ...
        tmpprt.SaveAs(fullfile(PRT.PRTSavePath,[subID '_MovT_Run1.prt']));
    end
    ...
    if PRT.VerbGeneration.Create == 1
        ...
        tmpprt.SaveAs(fullfile(PRT.PRTSavePath,[subID '_VerbGen_Run1.prt']));
    end
    ...
    if PRT.VFCategory.Create == 1
        ...
        tmpprt.SaveAs(fullfile(PRT.PRTSavePath,[subID '_VFCat_Run1.prt']));
    end
    ...
    if PRT.VFLetters.Create == 1
        ...
        tmpprt.SaveAs(fullfile(PRT.PRTSavePath,[subID '_VFLet_Run1.prt']));
    end
    ...
    if PRT.DefinitionNaming.Create == 1
        ...
        tmpprt.SaveAs(fullfile(PRT.PRTSavePath,[subID '_DefName_Run1.prt']));
    end
    % if user indicated they want to create a VF Category PRT.
